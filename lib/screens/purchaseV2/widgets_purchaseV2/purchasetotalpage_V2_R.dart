...
class SummaryAndPaymentSection_V2 extends StatefulWidget {
  const SummaryAndPaymentSection_V2({super.key});

  @override
  State<SummaryAndPaymentSection_V2> createState() =>
...
class _SummaryAndPaymentSectionState
    extends State<SummaryAndPaymentSection_V2> {
...
//หัวใจของการคำนวณ
  void _calculate() {
    ...
    // ใช้การจัดรูปแบบด้วยเครื่องหมายจุลภาค
    _calculationCtrls.beforeVat.text = formatNumber(beforeVat);
    _calculationCtrls.vatBase.text = formatNumber(beforeVat);
    _calculationCtrls.vatExtra.text = formatNumber(vatAmount);
    // แสดงมูลค่าสุทธิ (netAfterPrepay) = ยอดหลังหักส่วนลดสินค้าและส่วนลดก่อนชำระ
    _calculationCtrls.beforeVatAfterDiscount.text = formatNumber(netAfterPrepay);
    _calculationCtrls.netTotal.text = formatNumber(netAfterPrepay);
    _calculationCtrls.grandTotal.text = formatNumber(grandTotal);
