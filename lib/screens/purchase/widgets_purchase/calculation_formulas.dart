...
class CalculationFormulas {
  ...
  static String getVATFormulaExplanation(VatMode mode, double taxRate) {
    final rateStr = taxRate.toStringAsFixed(0);
  ...
  static String getVATCalculationExample(double baseAmount, double taxRate) {
    ...
    return '${baseAmount.toStringAsFixed(2)} × (${taxRate.toStringAsFixed(0)} ÷ 100) = ${vatAmount.toStringAsFixed(2)} บาท';
  }
  ...
  static String getVATCalculationExampleThai(
    ...
    final vatAmount = calculateVAT(baseAmount, taxRate);
    return '${formatThaiDecimal(baseAmount)} × (${taxRate.toStringAsFixed(0)} ÷ 100) = ${formatThaiCurrency(vatAmount)}';
  }
...
4. สกัดฐานภาษี: ${formatThaiCurrency(afterDiscount)} ÷ ${(1 + taxRatePercent / 100).toStringAsFixed(2)} = ${formatThaiCurrency(beforeVat)}
5. คำนวณ VAT: ${formatThaiCurrency(beforeVat)} × ${taxRatePercent.toStringAsFixed(0)}% = ${formatThaiCurrency(vatAmount)}
''';
    } else {
...
4. คำนวณ VAT: ${formatThaiCurrency(beforeVat)} × ${taxRatePercent.toStringAsFixed(0)}% = ${formatThaiCurrency(vatAmount)}
